...
public class PlayerControler : MonoBehaviour
{
    ...
    ["Header"("FreezeSkill Settings")]
    
    [SerializeField] public float freezeDuration;
    [serializeField] public float stunDuration;
    [serializeField] public float damageMultiplier;
    [serializeField] public float cooldown;
    [SerializeField] public float ManatoDeployedFrezeeSkill;

    //settings form prefab
...
public class FreezeSkill : MonoBehaviour
{
   ...
   [SerializeField] public float stunDuration = 3.0f;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.F) && !isOnCooldown && Math.Clamp("Mana") >= ManaToDeployedFreezeSkill)
                // press F to deployed
        {
            Freeze();
        }
        else if (isOnCooldown)
        {
            ...
            Debug.WriteLine("Cooldown:" + cooldown.ToString()) + "s";
        }
    }
